...
class Ion_auth_model extends CI_Model {
     ...
     * @author Mathew
     * */
    public function users_groups($search_val = NULL, $id = NULL) {
        ...
        //if query return not empty records
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $result) {
                $results = array();
                //fetch all data and store on data variable
                $data[] = array(
                    'id' => $result->id,
                    'name' => $result->name,
                ...
                //store all data into result variable.
                $results['data'] = $data;
                //total record found in table.
    ...
    public function getUserDetails($search_val = NULL, $id = null) {

        $this->db
                ->select($this->db->dbprefix('users') . ".id as id, first_name, last_name, email, phone, " . $this->db->dbprefix('groups') . ".name, active")
                ->from("users")
        ...
        if (!empty($search_val)) {
            $this->db->like('first_name', $search_val);
            $this->db->or_like('last_name', $search_val);
            $this->db->or_like('email', $search_val);
            $this->db->or_like('phone', $search_val);
            $this->db->or_like('name', $search_val);
            $this->db->or_like('active', $search_val);
    ...
    public function getUserDetailsById($id = null) {
        $this->db
                ->select($this->db->dbprefix('users') . ".id as id, first_name, last_name,username, gender, email, phone, company, " . $this->db->dbprefix('groups') . ".id as group_user_id, active")
                ->from("users")
                ->join('users_groups', 'users.id=users_groups.user_id', 'left')
                ->join('groups', 'users_groups.group_id=groups.id', 'left')
                ->where('users.id', $id);
    ...
    public function user_delete_by_id($id) {
        ...
        if ($res) {
            ...
            $grp = $this->db->delete('users_groups');
            if ($grp) {
                return TRUE;
            }
        } else {
    ...
    public function group_delete_by_id($id) {
        $this->db->where('id', $id);
        $response = $this->db->delete('groups');
        if ($response) {
            return TRUE;
        } else {
            return FALSE;
    ...
    public function register($identity, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        $group_id = $additional_data['group'];
        ...
        if (!empty($id)) {
            $user_group_data = array('user_id' => $id, 'group_id' => $group_id);
            $this->db->insert('users_groups', $user_group_data);
        }
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        if ($this->is_time_locked_out($identity)) {
            ...
            //I use md5 for future if want to reset password md5 is easy to reset
            //$this->md5($password);
    ...
    public function getUserGroup($id){
         $result = $this->db->select('id,user_id,group_id')
                            ->from('users_groups')
    ...
    public function updateUser($id, array $data) {
        ...
        if (array_key_exists($this->email, $data) && $this->identity_check($data[$this->email]) && $user->{$this->email} !== $data[$this->email]) {
        ...
        $dataInsert  = array('group_id' => $group_id, 'user_id'=>$id);
        unset($data['group']);

        $updateUser = $this->db->update($this->tables['users'], $data, array('id' => $id));
        
        if((empty($this->getUserGroup($id)) && !empty($group)) && $updateUser){
            $this->db->insert($this->tables['users_groups'], $dataInsert);
            $this->trigger_events(array('post_update_user', 'post_update_user_successful'));
            $this->set_message('update_successful');
            return TRUE;
        }
        
        //if ($updateUser && !empty($group)) {
        if((!empty($this->getUserGroup($id)) && !empty($group)) && $updateUser){
            $this->db->update($this->tables['users_groups'], $group, array('user_id' => $id));
            $this->trigger_events(array('post_update_user', 'post_update_user_successful'));
            $this->set_message('update_successful');
            return TRUE;
            //return $data = array('sucess'=>TRUE,'message'=>'update_successful');
        } else if ($updateUser) {
            $this->trigger_events(array('post_update_user', 'post_update_user_successful'));
            $this->set_message('update_successful');
            return TRUE;
            ...
            return FALSE;
            //return $data = array('sucess'=>FALSE,'message'=>'update_successful');
        }
    }

    public function update1($id, array $data) {
        ...
        if (array_key_exists($this->email, $data) || array_key_exists('password', $data)) {
    ...
    /*     * ************************ start Get user details datatable query ******************************** */



    /*     * ************************ end Get user details datatable query ******************************** */
}
